...
/**
 * Send automated email to client notifying them about a new invoice issued
 */
export async function sendInvoiceEmail({
  ...
  const resend = getResendClient();
  const subject = `📄 Invoice Tagihan Proyek (${invoiceNumber}) - ${projectName}`;
  const baseUrl = process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000';
  const portalUrl = `${baseUrl}/portal`;
    ...
    <html>
      <head>
        ...
        <style>
          ...
          .card { max-width: 580px; margin: 0 auto; background: #ffffff; border-radius: 20px; padding: 36px; border: 1px solid #e2e8f0; box-shadow: 0 10px 25px rgba(0,0,0,0.05); }
          .header { text-align: center; margin-bottom: 28px; padding-bottom: 20px; border-bottom: 1px solid #f1f5f9; }
          .badge { display: inline-block; background: #fef3c7; color: #d97706; font-size: 11px; font-weight: 700; padding: 4px 14px; border-radius: 99px; margin-top: 10px; text-transform: uppercase; letter-spacing: 0.5px; }
          .content { font-size: 14px; line-height: 1.6; color: #334155; }
          .invoice-box { background: #f8fafc; border: 1px solid #cbd5e1; border-radius: 12px; padding: 20px; margin: 24px 0; }
          .invoice-num { font-weight: 800; font-size: 16px; color: #0f172a; }
          .amount { font-size: 24px; font-weight: 900; color: #2563eb; margin: 8px 0; }
          .btn-container { text-align: center; margin: 32px 0; }
          .btn { background-color: #2563eb; color: #ffffff !important; font-size: 13px; font-weight: 800; text-decoration: none; padding: 14px 28px; border-radius: 14px; display: inline-block; box-shadow: 0 4px 12px rgba(37,99,235,0.25); text-transform: uppercase; letter-spacing: 0.5px; }
          .footer { font-size: 11px; color: #94a3b8; text-align: center; margin-top: 32px; border-top: 1px solid #f1f5f9; padding-top: 16px; }
        </style>
      ...
      <body>
        <div class="card">
          ...
          <div class="header">
            ...
            <div>
              <span class="badge">Invoice Tagihan Baru</span>
            </div>
          </div>
          
          <div class="content">
            <p>Halo <strong>${name}</strong>,</p>
            <p>Invoice pembayaran baru telah diterbitkan untuk proyek Anda <strong>"${projectName}"</strong>.</p>
            
            <div class="invoice-box">
              <div class="invoice-num">Nomor Invoice: ${invoiceNumber}</div>
              <div>Jatuh Tempo: <strong>${dueDate}</strong></div>
              <div class="amount">${formattedTotal}</div>
            </div>

            <p>Anda dapat mengunduh rincian faktur PDF dan mengunggah bukti pembayaran secara langsung melalui Client Portal.</p>
            
            <div class="btn-container">
              <a href="${portalUrl}" class="btn" target="_blank">Lihat Invoice & Bayar</a>
            </div>
          </div>
  ...
  if (resend) {
    try {
      ...
      if (response.error) {
        console.warn(`[Resend Invoice Email Fail] ${response.error.message}`);
        return { success: false, error: response.error.message };
      }

      console.log(`[Resend] Invoice email sent successfully to ${to}:`, response.data?.id);
      return { success: true, messageId: response.data?.id };
    } catch (error: any) {
      console.error(`[Resend Error] Failed sending invoice email to ${to}:`, error?.message || error);
      return { success: false, error: error?.message || 'Email delivery failed' };
    }
  } else {
    console.log(`\n==================================================`);
    console.log(`[SIMULATED INVOICE EMAIL - RESEND_API_KEY NOT SET]`);
    console.log(`To: ${to}`);
    console.log(`Subject: ${subject}`);
