...
export default function EditInsightPage() {
            ...
            fetch(`/api/admin/insights/${id}`)
              ...
              .then((res) => {
                if (res.success && res.insight) {
                  setArticle(res.insight);
                } else {
                  router.replace('/portal/insights');
                }
              })
              .catch(() => {
                router.replace('/portal/insights');
  ...
  if (isLoading || !userSession || userSession.role !== 'ADMIN' || !article) {
    return (
      <div className="h-screen bg-[#f0f4f8] flex flex-col items-center justify-center space-y-3">
        <Loader2 className="w-8 h-8 text-blue-600 animate-spin" />
        <span className="text-xs font-mono text-slate-500">Memuat data artikel untuk diedit...</span>
      </div>
    );
  }
  ...
  return (
    <div className="h-screen bg-[#f0f4f8] text-slate-900 p-3 sm:p-5 flex gap-0 lg:gap-5 font-sans antialiased w-full overflow-hidden">
      ...
      <main className="flex-1 flex flex-col min-w-0 h-[calc(100vh-2.5rem)] overflow-y-auto pr-1">
        ...
        <div className="mt-4">
          <InsightEditorView mode="edit" initialData={article} insightId={id} />
        </div>
      </main>
